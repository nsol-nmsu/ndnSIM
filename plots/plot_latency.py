...
def main(infile):
    ...
    #plt.xscale('log')
    lgd = plt.legend(loc='upper center', prop={'size':6}, bbox_to_anchor=(0.5,1.15), ncol=4, fancybox=True, shadow=True)
    fig = plt.gcf()
    fig.set_size_inches(4.5, 2.8)
    plt.subplots_adjust(left=0.16, right=0.96, top=0.95, bottom=0.22)
    fig.savefig("%s.pdf" % infile[:-4], dpi=120, bbox_extra_artists=(lgd,), bbox_inches='tight')
